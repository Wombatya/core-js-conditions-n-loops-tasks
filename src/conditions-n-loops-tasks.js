...
function convertToRomanNumerals(num) {
  ...
  const usualNum = [10, 9, 5, 4, 1];
  if (num === 0) {
    return '';
  }
  for (let i = 0; i < usualNum.length; i += 1) {
    if (num >= usualNum[i]) {
      return roman[i] + convertToRomanNumerals(num - usualNum[i]);
    }
  }
  return ':)';
}
...
function getBalanceIndex(arr) {
  ...
  for (let i = 0; i < arr.length - 2; i += 1) {
    let right = 0;
    left += arr[i];
    for (let j = i + 2; j <= arr.length - 1; j += 1) {
      right += arr[j];
    }
    if (left === right) {
      return i + 1;
...
function getSpiralMatrix(size) {
  ...
  while (firstRow <= lastRow && firstColumn <= lastColumn) {
    ...
    for (let k = firstRow; k <= lastRow; k += 1) {
      arr[k][lastColumn] = insertNum;
      insertNum += 1;
    }

    lastColumn -= 1;

    for (let l = lastColumn; l >= firstColumn; l -= 1) {
      arr[lastRow][l] = insertNum;
      insertNum += 1;
    }

    lastRow -= 1;

    for (let m = lastRow; m >= firstRow; m -= 1) {
      arr[m][firstColumn] = insertNum;
      insertNum += 1;
    }
    firstColumn += 1;
  }
